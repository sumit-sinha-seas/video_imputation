...
def run_experiment(args):
    ...
    if os.path.isfile(args.base_dir + "/Test_Batches_{}_{}.pkl".format(vid_lt, tmax*2)):
        ...
        make_batch = lambda s: Make_Video_batch(tmax=2*tmax, px=px, py=py, lt=vid_lt, batch=batch, seed=s, r=r)
        Test_Batches = [make_batch(s) for s in range(10)]
        with open(args.base_dir + "/Test_Batches_{}_{}.pkl".format(vid_lt, 2*tmax), "wb") as f:
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            for t in range(args.steps):
                ...
                if g_s % 1000 == 0:
                    ...
                    TT = TT[:,::2]
                    TD = TD[:,::2]
                    p_m_i, p_v_i = sess.run([p_m, p_v], {vid_batch: TD, beta: args.beta0})

                    _, _, MSE, _ = MSE_rotation(p_m_i, TT, p_v_i)
                    print('MSE : {}'.format(MSE))

                    if args.save:
                        new_res = sess.run(res_vars, {vid_batch: TD, beta: args.beta0})
                        new_res += [MSE, args.beta0, time.time()]
                        res_saver(new_res)

                # show plot and occasionally save
                if g_s % 1000 == 0 and (args.save or args.show_pics):
                    # [[ax_ij.clear() for ax_ij in ax_i] for ax_i in ax]
                    TT, TD = Test_Batches[0]
                    TT = TT[:,::2]
                    TD = TD[:,::2]
                    reconpath, reconvar, reconvid = sess.run([p_m, p_v, pred_vid], {vid_batch:TD, beta:args.beta0})
